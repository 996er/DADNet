...
class Options():
    ...
    def __init__(self):
        ...
        self.parser.add_argument('--niter', type=int, default=10, help='number of epochs to train for')
        ...
        self.parser.add_argument('--incremental_max_epochs', type=int, default=20,help='Maximum epochs for each incremental retraining round.')
        self.parser.add_argument('--incremental_max_minutes', type=int, default=60, help='Maximum wall-clock minutes for each incremental retraining round.')
        self.parser.add_argument('--incremental_max_samples', type=int, default=1024, help='Maximum retained samples used by each incremental retraining round.')
        self.parser.add_argument('--recent_window_size', type=int, default=1024, help='Sliding-window size for recent samples.')
        self.parser.add_argument('--history_per_class', type=int, default=64, help='Historical representative samples retained for each frequent class.')
        self.parser.add_argument('--validation_per_class', type=int, default=4, help='Held-out validation samples per class when materializing datasets.')
        self.parser.add_argument('--early_stop_patience', type=int, default=10, help='Early stopping patience measured in epochs without validation precision improvement.')
        self.parser.add_argument('--finetune_last_layers', type=int, default=1, help='How many tail blocks per sub-network stay trainable during incremental retraining.')
        self.parser.add_argument('--high_freq_drop_threshold', type=float, default=0.75, help='High-confidence threshold for treating a sample as high-frequency and eligible for filtering.')
        self.isTrain = True
        self.opt = None
