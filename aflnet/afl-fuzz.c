...
// // extern s32 shm_info_data_id[];
// // extern u8 *temp_out_buf[];
// extern s32 temp_out_buf_len[];
// extern int *info_bits;               
// extern u8 *info_data_bit[];
// extern u8 train_flag;
// extern u32 TRAIN_DATASET_NUM;
...
EXP_ST u64 total_crashes,             /* Total number of crashes          */
           ...
           cycles_train_finds_begin,
           cycles_train_finds_end,
           cycles_train_finds_gap,
           min_wo_finds;
...
extern int *info_bits              ;                /* SHM with info bitmap  */
extern u8 *info_data_bit[]         ;                /* SHM with info bitmap  */
extern u32 TRAIN_DATASET_NUM       ;
extern u8 train_flag               ;   
extern u8 DL_mode                  ;
extern u32 infer_flag              ,
success_infer_num                  ,
success_flag                       ,          /* the flag of the success          */
last_new_path                      ,
train_num                          ,
cur_hash                           ,
restart_flag                       ;

extern u64 infer_run               ,
infer_no_run                       ,
success_time                       ,
succ_infer                         ;
extern s32 val_pid                 ,                /* PID of the inference           */
rcv_pid                            ,
train_pid                          ;   

static void splice_msg(u8* out_buf, u32 len);
static u64 get_cur_time(void);

/* One execution record stores the path hash observed for a completed testcase
   together with a 0/1 flag that tells us whether this testcase produced new
   coverage. We keep these in a ring buffer so that recent-path statistics can
   be updated online without walking the whole corpus. */
   /* 每个执行记录存储一个已完成测试用例的路径哈希值，
   以及一个 0/1 标志，用于指示该测试用例是否产生了新的
   覆盖率。我们将这些数据保存在环形缓冲区中，以便能够
   在线更新近期路径统计信息，而无需遍历整个数据集。 */
   
typedef struct {
  u32 path_hash;
  u8 new_edges;
} adaptive_exec_record_t;

/* Validation records are produced only for samples that the filter actually
   judged as "drop candidates". After the testcase is still executed in shadow
   mode, we learn whether dropping it would have lost a useful sample and
   whether it belonged to a high-frequency path class. */
   /* 仅针对过滤器实际
   判定为“剔除候选”的样本生成验证记录。在测试用例仍以影子
   模式执行后，我们便能得知剔除该样本是否会导致有用的样本丢失，
   以及该样本是否属于高频路径类。 */
typedef struct {
  u8 useful;
  u8 high_freq;
  u8 novel;
  u8 filtered;
} adaptive_validation_record_t;

static adaptive_exec_record_t *adaptive_exec_window;
static adaptive_validation_record_t *adaptive_validation_window;
static u32 adaptive_exec_head, adaptive_exec_count;
static u32 adaptive_validation_head, adaptive_validation_count;
static u32 adaptive_validation_useful_total;
static u32 adaptive_validation_high_freq_total;
static u32 adaptive_validation_high_freq_useful_total;
static u32 adaptive_validation_novel_total;
static u32 adaptive_validation_novel_useful_total;
static u32 adaptive_high_freq_candidate_total;
static u32 adaptive_high_freq_filtered_total;
static u8 adaptive_gate_initialized;
static u8 adaptive_pending_validation;
static u8 adaptive_pending_gain_measure;
static double adaptive_current_filter_ratio;
static u32 adaptive_retrain_trigger_streak;
static u64 adaptive_last_retrain_time_ms;

/* Training-distribution entries compact the path histogram that represents the
   most recent model-training corpus. We compare future recent-path windows
   against this histogram using Jensen-Shannon divergence to decide whether the
   deployed model is drifting away from the workload it was trained on. */
   /* 训练-分布条目会压缩代表
   最新模型训练语料库的路径直方图。我们使用詹森-香农散度将未来的近期路径窗口
   与该直方图进行比较，以此判断
   已部署的模型是否偏离了其训练所基于的工作负载。 */
typedef struct {
  u32 path_hash;
  u32 count;
} adaptive_hist_entry_t;

static adaptive_hist_entry_t *adaptive_training_hist;
static adaptive_hist_entry_t *adaptive_staging_training_hist;
static u32 adaptive_training_hist_count;
static u32 adaptive_staging_training_hist_count;
static u8 *adaptive_gain_window;
static u32 adaptive_gain_head, adaptive_gain_count, adaptive_gain_sum;

static void adaptive_reset_staging_training_distribution(void);
...
static double adaptive_high_freq_filter_recall(void);
static double adaptive_target_filter_ratio(void);

static double adaptive_clamp_unit(double value) {
...
static u8 adaptive_coin_flip(double probability) {
...
static void adaptive_gate_reset_model_state(void) {

  /* These counters belong to the currently deployed model, not to the whole
     fuzzing campaign. Whenever we retrain and swap in a new model, validation
     precision, marginal gain and filter ratio must be recalibrated from zero
     so that stale statistics do not bias the next gating period. */
       /* 这些计数器属于当前部署的模型，而非整个
     模糊测试活动。每当我们重新训练并替换为新模型时，验证
     精度、边际增益和过滤比率都必须从零开始重新校准，
     以免过时的统计数据影响下一个门控周期。 */
  adaptive_validation_head = 0;
  adaptive_validation_count = 0;
  adaptive_validation_useful_total = 0;
  adaptive_validation_high_freq_total = 0;
  adaptive_validation_high_freq_useful_total = 0;
  adaptive_validation_novel_total = 0;
  adaptive_validation_novel_useful_total = 0;
  adaptive_high_freq_candidate_total = 0;
  adaptive_high_freq_filtered_total = 0;
  adaptive_pending_validation = 0;
  adaptive_pending_gain_measure = 0;
  adaptive_gain_head = 0;
  adaptive_gain_count = 0;
  adaptive_gain_sum = 0;
  adaptive_current_filter_ratio = adaptive_clamp_unit(gate_base_filter_ratio);
  adaptive_retrain_trigger_streak = 0;

}

static void adaptive_gate_init(void) {

  if (adaptive_gate_initialized) return;

  /* Defensive normalization keeps the online control logic alive even if the
     config file forgets to provide a value or sets it to zero. */
       /* 防御性归一化确保即使配置文件忘记提供某个值或将其设为零，
     在线控制逻辑仍能正常运行。 */
  if (!gate_window_size) gate_window_size = 256;
  if (!gate_validation_window) gate_validation_window = 64;
  if (!gate_topk) gate_topk = 1;
  if (!gate_validation_min_samples) gate_validation_min_samples = 1;
  if (!gate_high_freq_validation_min_samples) gate_high_freq_validation_min_samples = 1;

  adaptive_exec_window = ck_alloc(sizeof(adaptive_exec_record_t) * gate_window_size);
  adaptive_validation_window =
      ck_alloc(sizeof(adaptive_validation_record_t) * gate_validation_window);
  adaptive_training_hist = ck_alloc(sizeof(adaptive_hist_entry_t) * TRAIN_DATASET_NUM);
  adaptive_staging_training_hist = ck_alloc(sizeof(adaptive_hist_entry_t) * TRAIN_DATASET_NUM);
  adaptive_gain_window = ck_alloc(sizeof(u8) * retrain_gain_window);

  adaptive_gate_initialized = 1;
...
static void adaptive_reset_staging_training_distribution(void) {
  ...
  /* During data collection we build the histogram in a staging area first.
     Only after training succeeds do we "commit" it as the reference
     distribution for future drift / JSD checks. */
       /* 在数据收集过程中，我们会先在暂存区构建直方图。
     只有在训练成功后，才会将其“提交”为参考
     分布，用于后续的漂移/JSD检查。 */
  adaptive_staging_training_hist_count = 0;

}

static void adaptive_record_hist_sample(adaptive_hist_entry_t *hist, u32 *hist_count,
                                        u32 path_hash) {

  u32 i;

  /* The histogram is intentionally compact: one bucket per distinct path hash.
     This gives us an inexpensive approximate distribution over recently seen
     protocol behaviors without storing every raw sample path. */
       /* 该直方图设计得较为紧凑：每个不同的路径哈希对应一个桶。
     这样，我们无需存储每个原始样本路径，
     就能以较低的开销获得近期协议行为的近似分布。 */
  for (i = 0; i < *hist_count; ++i) {
    if (hist[i].path_hash == path_hash) {
      hist[i].count++;
      return;
    }
  }

  if (*hist_count >= TRAIN_DATASET_NUM) return;

  hist[*hist_count].path_hash = path_hash;
  hist[*hist_count].count = 1;
  (*hist_count)++;
...
static void adaptive_commit_training_distribution(void) {
  ...
  /* A committed histogram means "this is what the currently deployed model was
     ...
     baseline instead of the still-mutating collection buffer. */
       /* 已提交的直方图意味着“这是当前部署的模型所基于的
     训练数据”。重新训练会将实时流量与这个已冻结的
     基准进行比较，而不是与仍在变化的收集缓冲区进行比较。 */
  memcpy(adaptive_training_hist, adaptive_staging_training_hist,
         sizeof(adaptive_hist_entry_t) * adaptive_staging_training_hist_count);
  adaptive_training_hist_count = adaptive_staging_training_hist_count;
  adaptive_reset_staging_training_distribution();
...
static void adaptive_record_gain(u8 useful) {
  ...
  /* Marginal gain tracks how much useful coverage survives after filtering.
     ...
     when deciding whether the current filter is still paying for itself. */
       /* 边际增益用于追踪过滤后剩余的有用覆盖范围。
     我们将其存储在固定大小的环形缓冲区中，这样在判断当前过滤器是否仍物有所值时，
     仅需考虑最近的增益。 */
  if (adaptive_gain_count == retrain_gain_window)
    adaptive_gain_sum -= adaptive_gain_window[adaptive_gain_head];
  else
    adaptive_gain_count++;

  adaptive_gain_window[adaptive_gain_head] = useful ? 1 : 0;
  adaptive_gain_sum += useful ? 1 : 0;
  adaptive_gain_head = (adaptive_gain_head + 1) % retrain_gain_window;

}

static double adaptive_marginal_gain(void) {

  /* Returning 1.0 for an empty window avoids accidentally firing retraining
     before any post-filter executions have been observed. */
       /* 对于空窗口返回 1.0 可避免在观察到任何后过滤器执行之前意外触发重新训练。 */
  if (!adaptive_gain_count) return 1.0;
  return (double)adaptive_gain_sum / (double)adaptive_gain_count;

}

static u32 adaptive_exec_window_index(u32 logical_index) {

  u32 start = 0;
  if (adaptive_exec_count == gate_window_size) start = adaptive_exec_head;
  return (start + logical_index) % gate_window_size;

}

static adaptive_exec_record_t adaptive_exec_record_at(u32 logical_index) {

  return adaptive_exec_window[adaptive_exec_window_index(logical_index)];

}

static double adaptive_new_edge_rate_for_range(u32 start, u32 count) {

  u32 i, new_edges = 0;
  if (!count) return 0.0;

  /* The "new edge rate" signal is the first half of the gate: if the recent
     stream is still rich in new coverage, filtering should back off because we
     are exploring rather than exploiting. */
       /* “新边缘速率”信号是门控机制的前半部分：如果最近的
     数据流中仍包含大量新覆盖内容，则应暂停过滤，因为我们
     当前处于探索阶段而非利用阶段。 */
  for (i = 0; i < count; ++i)
    new_edges += adaptive_exec_record_at(start + i).new_edges ? 1 : 0;

  return (double)new_edges / (double)count;

}

static double adaptive_topk_ratio_for_range(u32 start, u32 count) {

  u32 i, j, filled = 0, topk_sum = 0;
  u32 *top_hashes, *top_counts;

  if (!count) return 0.0;

  top_hashes = ck_alloc(sizeof(u32) * gate_topk);
  top_counts = ck_alloc(sizeof(u32) * gate_topk);

  /* We approximate path concentration by summing the mass of the most common
     K path hashes inside the recent window. High concentration means the run is
     stuck in a few dominant modes, which makes learned filtering safer. */
       /* 我们通过求和最近时间窗口内最常见的
     K 个路径哈希值来近似路径集中度。高集中度意味着运行过程
     被困在少数几种主导模式中，这使得所学习的过滤机制更加安全。 */
  for (i = 0; i < count; ++i) {

    u32 path_hash = adaptive_exec_record_at(start + i).path_hash;
    u8 seen = 0;

    for (j = 0; j < i; ++j) {
      if (adaptive_exec_record_at(start + j).path_hash == path_hash) {
        seen = 1;
        break;
      }
    }
    ...
    u32 occurrences = 0;
    for (j = 0; j < count; ++j)
      if (adaptive_exec_record_at(start + j).path_hash == path_hash) occurrences++;

    if (filled < gate_topk) {
      top_hashes[filled] = path_hash;
      top_counts[filled] = occurrences;
      filled++;
    } else {
      u32 min_idx = 0;
      for (j = 1; j < gate_topk; ++j)
        if (top_counts[j] < top_counts[min_idx]) min_idx = j;

      if (occurrences > top_counts[min_idx]) {
        top_hashes[min_idx] = path_hash;
        top_counts[min_idx] = occurrences;
      }
    }
  }

  for (i = 0; i < filled; ++i) topk_sum += top_counts[i];

  ck_free(top_hashes);
  ck_free(top_counts);

  return (double)topk_sum / (double)count;

}

/* This helper is used when we evaluate validation quality for "high-frequency"
   classes. Those classes are exactly the ones most likely to dominate compute
   cost, so we track their precision separately from the global precision. */
   /* 该辅助函数用于评估“高频”
   类的验证质量。这些类恰恰是最可能占据计算
   成本的主力，因此我们将其精度与全局精度分开进行追踪。 */
static u8 adaptive_path_is_topk_in_range(u32 path_hash, u32 start, u32 count) {
  ...
  for (i = 0; i < count; ++i) {
    ...
    u32 current_hash = adaptive_exec_record_at(start + i).path_hash;
    u8 seen = 0;

    for (j = 0; j < i; ++j) {
      if (adaptive_exec_record_at(start + j).path_hash == current_hash) {
        seen = 1;
        break;
      }
    }
    ...
    u32 occurrences = 0;
    for (j = 0; j < count; ++j)
      if (adaptive_exec_record_at(start + j).path_hash == current_hash) occurrences++;

    if (filled < gate_topk) {
      top_hashes[filled] = current_hash;
      top_counts[filled] = occurrences;
      filled++;
    } else {
      u32 min_idx = 0;
      ...
      if (occurrences > top_counts[min_idx]) {
        top_hashes[min_idx] = current_hash;
        top_counts[min_idx] = occurrences;
      }
    }
  }
...
static double adaptive_validation_precision(void) {

  /* Here "precision" means: among the samples that the model wanted to filter
     out, how many were truly safe to drop after online validation. Because
     "useful" marks false positives, precision is 1 - FP rate. */
       /* 这里“精确率”的含义是：在模型试图过滤掉的样本中，
     经过在线验证后，有多少样本确实可以安全地舍弃。由于
     “有用”标记的是假阳性，因此精确率等于 1 减去假阳性率。 */
  if (!adaptive_validation_count) return 0.0;
  return 1.0 - ((double)adaptive_validation_useful_total /
                (double)adaptive_validation_count);

}

static double adaptive_high_freq_validation_precision(void) {

  if (!adaptive_validation_high_freq_total) return 0.0;
  return 1.0 - ((double)adaptive_validation_high_freq_useful_total /
                (double)adaptive_validation_high_freq_total);

}

static double adaptive_validation_false_positive_risk(void) {

  if (!adaptive_validation_count) return 0.0;
  return (double)adaptive_validation_useful_total / (double)adaptive_validation_count;

}

static double adaptive_high_freq_false_positive_risk(void) {

  if (!adaptive_validation_high_freq_total) return 0.0;
  return (double)adaptive_validation_high_freq_useful_total /
         (double)adaptive_validation_high_freq_total;
...
static double adaptive_novel_false_drop_rate(void) {
  ...
  if (!adaptive_validation_novel_total) return 0.0;
  return (double)adaptive_validation_novel_useful_total /
         (double)adaptive_validation_novel_total;

}

static double adaptive_high_freq_filter_recall(void) {

  if (!adaptive_high_freq_candidate_total) return 0.0;
  return (double)adaptive_high_freq_filtered_total /
         (double)adaptive_high_freq_candidate_total;

}

static double adaptive_path_jsd(void) {

  u32 i, j, recent_count = 0;
  double jsd = 0.0, recent_total = 0.0, training_total = 0.0;
  ...
  recent_hist = ck_alloc(sizeof(adaptive_hist_entry_t) * recent_window);

  /* Jensen-Shannon divergence compares the recent path distribution with the
     distribution used by the currently deployed model. A large divergence is a
     strong signal that the model's training data no longer matches the live
     fuzzing workload, so incremental retraining should be scheduled. */
       /* 詹森-香农散度将近期路径分布与
     当前部署的模型所使用的分布进行比较。较大的散度是
     一个强烈的信号，表明模型的训练数据已不再与实际
     模糊测试工作负载相匹配，因此应安排增量重新训练。 */
  for (i = adaptive_exec_count - recent_window; i < adaptive_exec_count; ++i) {
    adaptive_exec_record_t record = adaptive_exec_record_at(i);
    u8 found = 0;

    for (j = 0; j < recent_count; ++j) {
      if (recent_hist[j].path_hash == record.path_hash) {
        recent_hist[j].count++;
        found = 1;
        break;
      }
    ...
    if (!found) {
      recent_hist[recent_count].path_hash = record.path_hash;
      recent_hist[recent_count].count = 1;
      recent_count++;
    }
  }

  for (i = 0; i < recent_count; ++i) recent_total += recent_hist[i].count;
  for (i = 0; i < adaptive_training_hist_count; ++i) training_total += adaptive_training_hist[i].count;

  for (i = 0; i < recent_count; ++i) {
    double p = recent_hist[i].count / recent_total;
    double q = 0.0;

    for (j = 0; j < adaptive_training_hist_count; ++j) {
      if (adaptive_training_hist[j].path_hash == recent_hist[i].path_hash) {
        q = adaptive_training_hist[j].count / training_total;
        break;
      }
    }
  ...
  for (i = 0; i < adaptive_training_hist_count; ++i) {
    double q = adaptive_training_hist[i].count / training_total;
    double p = 0.0;

    for (j = 0; j < recent_count; ++j) {
      if (recent_hist[j].path_hash == adaptive_training_hist[i].path_hash) {
        p = recent_hist[j].count / recent_total;
        break;
      }
    }
...
static double adaptive_target_filter_ratio(void) {
  ...
  double ratio = adaptive_clamp_unit(gate_base_filter_ratio);
  double topk_ratio;
  double new_edge_rate;
  double novel_false_drop_rate;
  double high_freq_filter_recall;
  ...
  new_edge_rate = adaptive_new_edge_rate_for_range(0, adaptive_exec_count);
  novel_false_drop_rate = adaptive_novel_false_drop_rate();
  high_freq_filter_recall = adaptive_high_freq_filter_recall();
  validation_precision = adaptive_validation_precision();
  high_freq_precision = adaptive_high_freq_validation_precision();

  /* Dynamic filtering deliberately stays on by default. The ratio moves up
     aggressively when the workload is dominated by top-K paths and moves down
     when exploration rises or novel samples are at risk of being dropped. */
  if (topk_ratio >= gate_topk_high_threshold) ratio += 0.30;
  if (topk_ratio >= gate_topk_ratio_threshold) ratio += 0.15;
  if (new_edge_rate >= gate_new_edge_high_threshold) ratio -= 0.40;
  if (novel_false_drop_rate >= gate_novel_guard_threshold) ratio -= 0.40;
  if (high_freq_filter_recall >= gate_high_freq_recall_threshold) ratio += 0.20;
  if (validation_precision >= gate_precision_threshold &&
      high_freq_precision >= gate_high_freq_precision_threshold)
    ratio += 0.10;

  if (ratio < gate_min_filter_ratio) ratio = gate_min_filter_ratio;
  if (ratio > gate_filter_ratio) ratio = gate_filter_ratio;

  adaptive_current_filter_ratio = adaptive_clamp_unit(ratio);
  return adaptive_current_filter_ratio;

}

static void adaptive_record_execution(u32 path_hash, u8 new_edges) {
  ...
  /* Every executed testcase contributes one record to the recent window. This
     ...
     JSD, and gain accounting. */
       /* 每个执行的测试用例都会向近期窗口添加一条记录。该
     窗口是边缘速率检查、前K位占用率、
     JSD 和增益核算的共同数据源。 */
  slot = &adaptive_exec_window[adaptive_exec_head];

  if (adaptive_exec_count < gate_window_size) adaptive_exec_count++;

  slot->path_hash = path_hash;
  slot->new_edges = new_edges;

  adaptive_exec_head = (adaptive_exec_head + 1) % gate_window_size;

}

static void adaptive_adjust_filter_ratio(void) {

  double min_ratio = adaptive_clamp_unit(gate_min_filter_ratio);
  double step = adaptive_clamp_unit(gate_filter_ratio_step);
  double fp_risk = adaptive_validation_false_positive_risk();
  ...
  /* False-positive risk should degrade the filter smoothly instead of turning
     ...
     precision recovers, we slowly restore it back toward the configured base. */
       /* 误报风险应使过滤器平滑地降低敏感度，而非直接
     将其完全关闭。当风险上升时，我们降低实际的过滤比率；当
     准确率恢复时，我们将其缓慢恢复至配置的基准值。 */
  if (fp_risk > gate_validation_false_positive_threshold ||
      hf_fp_risk > gate_validation_false_positive_threshold ||
      novel_fp_risk > gate_novel_guard_threshold) {
    adaptive_current_filter_ratio -= step;
    if (adaptive_current_filter_ratio < min_ratio)
      adaptive_current_filter_ratio = min_ratio;
    return;
  }
  adaptive_target_filter_ratio();

}

static void adaptive_record_validation(u8 useful, u8 high_freq, u8 novel) {
  ...
  /* This ring buffer stores the outcome of shadow validation runs. The same
     ...
     and the adaptive filter-ratio controller. */
       /* 此环形缓冲区用于存储影子验证运行的结果。同一批
     样本会同时更新全局精度、高频精度
     以及自适应滤波比控制器。 */
  slot = &adaptive_validation_window[adaptive_validation_head];

  if (adaptive_validation_count == gate_validation_window) {
    adaptive_validation_useful_total -= slot->useful ? 1 : 0;
    if (slot->high_freq) {
      adaptive_validation_high_freq_total--;
      adaptive_validation_high_freq_useful_total -= slot->useful ? 1 : 0;
    }
    if (slot->novel) {
      adaptive_validation_novel_total--;
      adaptive_validation_novel_useful_total -= slot->useful ? 1 : 0;
    }
  } else {
    adaptive_validation_count++;
  }

  slot->useful = useful;
  slot->high_freq = high_freq;
  slot->novel = novel;
  slot->filtered = 0;

  adaptive_validation_useful_total += useful ? 1 : 0;
  if (high_freq) {
    adaptive_validation_high_freq_total++;
    adaptive_validation_high_freq_useful_total += useful ? 1 : 0;
  }
  if (novel) {
    adaptive_validation_novel_total++;
    adaptive_validation_novel_useful_total += useful ? 1 : 0;
  }
  adaptive_validation_head = (adaptive_validation_head + 1) % gate_validation_window;
  adaptive_adjust_filter_ratio();

}

static u8 adaptive_should_retrain() {

  u8 triggered = 0;
  u64 now_ms = get_cur_time();

  /* The new retraining policy intentionally ignores the old time-based trigger.
     We retrain only when one of the online quality signals says the deployed
     model is no longer aligned with current fuzzing behavior. */
  ...
  if (adaptive_path_jsd() >= retrain_jsd_threshold) triggered = 1;

  if (adaptive_gain_count >= retrain_gain_window / 2 &&
      adaptive_marginal_gain() <= retrain_marginal_gain_threshold)
    triggered = 1;

  if (triggered) adaptive_retrain_trigger_streak++;
  ...
  adaptive_retrain_trigger_streak = 0;
  adaptive_last_retrain_time_ms = now_ms;
  return 1;

}

static u8 adaptive_request_inference(u8 *candidate_buf, u32 candidate_len,
                                     u8 *should_execute) {

  /* This helper centralizes the shared-memory round-trip with the Python model.
     It is shared by both "real filtering" and "shadow validation" so the call
     path stays identical and the statistics remain comparable. */
       /* 此辅助函数将与 Python 模型的共享内存往返通信集中管理。
     它同时被“真实滤波”和“影子验证”共享，因此调用
     路径保持一致，统计数据也具有可比性。 */
  if (!(DL_mode && success_flag && !restart_flag && candidate_len < SHM_DATA_SIZE))
    return 0;

  splice_msg(candidate_buf, candidate_len);

  memset(info_data_bit[0], 0, SHM_DATA_SIZE);
  save_kl_messages_to_shm(kl_messages);

  while (1) {
    if (infer()) {
      *should_execute = *info_bits ? 1 : 0;
      return 1;
    }
  }
...
void save_state_aware_to_train_file(){
  ...
  u8 *fname = alloc_printf("%s/collection/1/id:%06u,%s", out_dir, train_num,temp_str);
...
void splice_msg(u8* out_buf, u32 len){
  u32 region_count = 0;
...
void save_kl_messages_to_shm(klist_t(lms) *kl_messages)
{
  ...
  //将拼接好的message存入共享内存
  if (len > SHM_DATA_SIZE) len = SHM_DATA_SIZE ;
  memcpy(info_data_bit[0],mem,len);
...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault) {
  ...
  if (fault == crash_mode) {
    ...
    if (!hnb) {
      ...
      if (DL_mode){//DL模式下,训练未完成的状态下才能够进行AI引导
        ...
        if (train_num < TRAIN_DATASET_NUM && !strcmp(stage_name, "havoc")) {
          ...
          fn = alloc_printf("%s/collection/1/id:%06u,%s,%u", out_dir, train_num, describe_op(hnb), cur_hash);

          #else

          fn = alloc_printf("%s/collection/1/id_%06u", out_dir, queued_paths);
        ...
        //收集完种子进行训练
        //种子数量达到阈值，训练完成标志为0，才能进行训练，训练标志为1则退出模型训练
        if (train_num == TRAIN_DATASET_NUM && success_flag == 0 ) {

          //训练成功后，设置训练完成标志，以免每次都进来模型训练
          if (train_model() == 1){
          ...
          else if (train_model() == -1) PFATAL("Failed to fork train");
    ...
    fn = alloc_printf("%s/queue/id:%06u,%s,%u", out_dir, queued_paths,
                      describe_op(hnb),cur_hash);
    ...
    if (DL_mode) {
        ...
        // queue_top->exec_cksum = cur_hash;

        if (succ_infer) {
            queue_top->has_infer = 1;  //预测正确
            succ_infer = 0;
        }
    }
    // else{
...
/***********************************************************/
  ...
  fn = alloc_printf("%s/collection/1/", out_dir);
  if (delete_files(fn, CASE_PREFIX)) goto dir_cleanup_failed;
  ck_free(fn);

  fn = alloc_printf("%s/collection", out_dir);
  if (delete_files(fn, CASE_PREFIX)) goto dir_cleanup_failed;
  ck_free(fn);

  fn = alloc_printf("%s/train/1/", out_dir);
  if (delete_files(fn, CASE_PREFIX)) goto dir_cleanup_failed;
  ck_free(fn);

  fn = alloc_printf("%s/train", out_dir);
  if (delete_files(fn, CASE_PREFIX)) goto dir_cleanup_failed;
  ck_free(fn);

  fn = alloc_printf("%s/test/1/", out_dir);
  if (delete_files(fn, CASE_PREFIX)) goto dir_cleanup_failed;
  ck_free(fn);

  fn = alloc_printf("%s/test/2/", out_dir);
  if (delete_files(fn, CASE_PREFIX)) goto dir_cleanup_failed;
  ck_free(fn);

  fn = alloc_printf("%s/test", out_dir);
  if (delete_files(fn, CASE_PREFIX)) goto dir_cleanup_failed;
  ck_free(fn);

  fn = alloc_printf("%s/val/1/.cur_input", out_dir);
  if (unlink(fn) && errno != ENOENT) goto dir_cleanup_failed;
  ck_free(fn);

  fn = alloc_printf("%s/val/1", out_dir);
  if (delete_files(fn, CASE_PREFIX)) goto dir_cleanup_failed;
  ck_free(fn);

  fn = alloc_printf("%s/val", out_dir);
  if (delete_files(fn, CASE_PREFIX)) goto dir_cleanup_failed;
  ck_free(fn);
...
#define SP20    SP10 SP10
  ...
  if (dumb_mode) {
    ...
    min_wo_finds = (cur_ms - last_path_time) / 1000 / 60;
    ...
    if (cycles_wo_finds > 100 && !pending_not_fuzzed && min_wo_finds > 120) strcpy(tmp, cLGN);
  ...
  SAYF(bV bSTOP "        trim : " cRST "%-37s" " !%d %d %s  %s  %s!" bSTG bVR bH20 bH2 bH2 bRB "\n"
       bLB bH30 bH20 bH2 bH bRB bSTOP cRST RESET_G1, tmp,restart_flag,train_flag,DI(success_infer_num),DI(infer_run),DI(infer_no_run));
...
havoc_stage:
  ...
  if (success_flag && adaptive_online_enable) {
    ...
    if (adaptive_should_retrain()){

      restart_train();

      restart_flag = 1; //restart train model
...
abandon_entry:
  ...
  if (!stop_soon && !queue_cur->cal_failed && !queue_cur->was_fuzzed) {
    ...
    pending_not_fuzzed--;
    if (queue_cur->favored && pending_favored > 0 ) pending_favored--;
  }
...
/************************************************************/
  ...
  tmp = alloc_printf("%s/collection", out_dir);
  if (mkdir(tmp, 0700)) PFATAL("Unable to create '%s'", tmp);
  ck_free(tmp);

  tmp = alloc_printf("%s/collection/1", out_dir);
  if (mkdir(tmp, 0700)) PFATAL("Unable to create '%s'", tmp);
  ck_free(tmp);
  ...
  tmp = alloc_printf("%s/train/1", out_dir);
  if (mkdir(tmp, 0700)) PFATAL("Unable to create '%s'", tmp);
  ck_free(tmp);

  tmp = alloc_printf("%s/test", out_dir);
  if (mkdir(tmp, 0700)) PFATAL("Unable to create '%s'", tmp);
  ck_free(tmp);

  tmp = alloc_printf("%s/test/1/", out_dir);
  if (mkdir(tmp, 0700)) PFATAL("Unable to create '%s'", tmp);
  ck_free(tmp);

  tmp = alloc_printf("%s/test/2/", out_dir);
  if (mkdir(tmp, 0700)) PFATAL("Unable to create '%s'", tmp);
  ck_free(tmp);

  tmp = alloc_printf("%s/val/", out_dir);
  if (mkdir(tmp, 0700)) PFATAL("Unable to create '%s'", tmp);
  ck_free(tmp);

  tmp = alloc_printf("%s/val/1/", out_dir);
  if (mkdir(tmp, 0700)) PFATAL("Unable to create '%s'", tmp);
  ck_free(tmp);
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:L:S:M:x:QN:D:W:w:e:P:KEgq:s:RFc:l:b:h:")) > 0)
    ...
    switch (opt) {
      ...
      //-----------------------------------------
      case 'L': /* deep learning mode */

          if (DL_mode) FATAL("Multiple -L options not supported");
          DL_mode = 1;
          setup_info_shm();
          parse_config();
          adaptive_gate_init();
      ...
      case 'g':
        if (state_aware_train_mode) FATAL("Multiple -E options not supported");
        state_aware_train_mode = 1;
        break;
  ...
  cycles_train_finds_begin = start_time;
